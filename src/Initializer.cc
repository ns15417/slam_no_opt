...
Initializer::Initializer(const Frame &ReferenceFrame, float sigma,
  ...
  const float coefA = 13.928 * mfAlpha * mfAlpha * mfBeta + 2 * mfAlpha -
                      1;  // 150 degrees coefiicient of equation
  const float coefB = 2 - 2 * mfAlpha;
  const float coefC = -1;

  mzMin =
      (-coefB + sqrt(coefB * coefB - 4 * coefA * coefC)) / (2 * coefA) - 0.1;
}
...
cv::Mat Initializer::Compute3DH21(const vector<cv::Point3f> &vP1,
                                  const vector<cv::Point3f> &vP2) {
  const int N = vP1.size();
...
cv::Mat Initializer::Compute3DF21(const vector<cv::Point3f> &vP1,
                                  const vector<cv::Point3f> &vP2) {
  const int N = vP1.size();
...
float Initializer::CheckHomography(const cv::Mat &H21, const cv::Mat &H12,
  ...
  const float invSigmaSquare =
      sigma * sigma;  // using EUCM model change the dimension ( 1/pixel -> 1/fx
                      // ), sigma = fx
  ...
  for (int i = 0; i < N; i++) {
    ...
    // lambda*x1 = H'*x2, lambda*x1a = x1b, lambda is computed by Least squares
    // method
    ...
    // (lambdaS1*u1-u2in1)*(lambdaS1*u1-u2in1)+(lambdaS1*v1-v2in1)*(lambdaS1*v1-v2in1)+(lambdaS1*w1-w2in1)*(lambdaS1*w1-w2in1);
    const float squareDist1 = (u1 - u2in1) * (u1 - u2in1) +
                              (v1 - v2in1) * (v1 - v2in1) +
                              (w1 - w2in1) * (w1 - w2in1);
    // const float squareDist1 =
    // (lambdaS1*u1-u2in1)*(lambdaS1*u1-u2in1)+(lambdaS1*v1-v2in1)*(lambdaS1*v1-v2in1);
    ...
    // lambda*x2 = H*x1, lambda*x2a = x2b, lambda is computed by Least squares
    // method
    ...
    // (lambdaS2*u2-u1in2)*(lambdaS2*u2-u1in2)+(lambdaS2*v2-v1in2)*(lambdaS2*v2-v1in2)+(lambdaS2*w2-w1in2)*(lambdaS2*w2-w1in2);
    const float squareDist2 = (u2 - u1in2) * (u2 - u1in2) +
                              (v2 - v1in2) * (v2 - v1in2) +
                              (w2 - w1in2) * (w2 - w1in2);
    // const float squareDist2 =
    // (lambdaS2*u2-u1in2)*(lambdaS2*u2-u1in2)+(lambdaS2*v2-v1in2)*(lambdaS2*v2-v1in2);
...
float Initializer::CheckFundamental(const cv::Mat &F21,
  ...
  const float thScore = 7.81; // TODO: original 5.991
...
bool Initializer::ReconstructF(vector<bool> &vbMatchesInliers, cv::Mat &F21,
  ...
  vP3D1, 4.0*mSigma2, vbTriangulated1, parallax1); int nGood2 =
  CheckRT(R2,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K,
  vP3D2, 4.0*mSigma2, vbTriangulated2, parallax2); int nGood3 =
  CheckRT(R1,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K,
  vP3D3, 4.0*mSigma2, vbTriangulated3, parallax3); int nGood4 =
  CheckRT(R2,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K,
  vP3D4, 4.0*mSigma2, vbTriangulated4, parallax4);*/

  int nGood1 = CheckRT3D(R1, t1, mvP3M1, mvP3M2, mvMatches12, vbMatchesInliers,
                         vP3D1, 4.0 * mSigma2, vbTriangulated1, parallax1);
  int nGood2 = CheckRT3D(R2, t1, mvP3M1, mvP3M2, mvMatches12, vbMatchesInliers,
                         vP3D2, 4.0 * mSigma2, vbTriangulated2, parallax2);
  int nGood3 = CheckRT3D(R1, t2, mvP3M1, mvP3M2, mvMatches12, vbMatchesInliers,
                         vP3D3, 4.0 * mSigma2, vbTriangulated3, parallax3);
  int nGood4 = CheckRT3D(R2, t2, mvP3M1, mvP3M2, mvMatches12, vbMatchesInliers,
                         vP3D4, 4.0 * mSigma2, vbTriangulated4, parallax4);
  ...
  int nMinGood = max(static_cast<int>(0.75 * N), minTriangulated);
  ...
  cout << "[Initializer] failed to Reconstruct F" << endl;
  return false;
}
...
bool Initializer::ReconstructH(vector<bool> &vbMatchesInliers, cv::Mat &H21,
  ...
  for (size_t i = 0; i < 8; i++) {
    ...
    // int nGood =
    // CheckRT(vR[i],vt[i],mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K,vP3Di, 4.0*mSigma2,
    // vbTriangulatedi, parallaxi);
    int nGood =
        CheckRT3D(vR[i], vt[i], mvP3M1, mvP3M2, mvMatches12, vbMatchesInliers,
...
void Initializer::TriangulateFisheye(const cv::Point3f &p1m,
                                     const cv::Point3f &p2m, const cv::Mat &P1,
...
void Initializer::Normalize3D(const vector<cv::Point3f> &vP3M,
                              vector<cv::Point3f> &vNormalizedPoints,
                              cv::Mat &T) {
  float meanX = 0;
  float meanY = 0;
...
int Initializer::CheckRT3D(const cv::Mat &R, const cv::Mat &t,
                           const vector<cv::Point3f> &vP3M1,
                           const vector<cv::Point3f> &vP3M2,
                           const vector<Match> &vMatches12,
                           vector<bool> &vbMatchesInliers,
                           vector<cv::Point3f> &vP3D, float th2,
                           vector<bool> &vbGood, float &parallax) {
  ...
  for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
    ...
    float im1d =
        sqrt(mfBeta * (p3dC1x * p3dC1x + p3dC1y * p3dC1y) + p3dC1z * p3dC1z);
    ...
    im1z = (1 - mfBeta * mfAlpha * mfAlpha * distance1R2) /
           (mfAlpha * sqrt(1 - (2 * mfAlpha - 1) * mfBeta * distance1R2) + 1 -
            mfAlpha);

    float squareError1 = (im1x - p1m.x) * (im1x - p1m.x) +
                         (im1y - p1m.y) * (im1y - p1m.y) +
                         (im1z - p1m.z) * (im1z - p1m.z);
    ...
    float im2d =
        sqrt(mfBeta * (p3dC2x * p3dC2x + p3dC2y * p3dC2y) + p3dC2z * p3dC2z);
    ...
    im2z = (1 - mfBeta * mfAlpha * mfAlpha * distance2R2) /
           (mfAlpha * sqrt(1 - (2 * mfAlpha - 1) * mfBeta * distance2R2) + 1 -
            mfAlpha);

    float squareError2 = (im2x - p2m.x) * (im2x - p2m.x) +
                         (im2y - p2m.y) * (im2y - p2m.y) +
                         (im2z - p2m.z) * (im2z - p2m.z);
...
void Initializer::ComputeLambdaH(const cv::Mat &H21, const cv::Mat &H12,
                                 const vector<cv::Point3f> &vP1,
                                 const vector<cv::Point3f> &vP2) {
  mLambdaH12 = 0;
  mLambdaH21 = 0;
