...
LoopClosing::LoopClosing(Map* pMap, KeyFrameDatabase* pDB, ORBVocabulary* pVoc,
                         const bool bFixScale, const string& strSettingPath)
  ...
  mnCovisibilityConsistencyTh =
      fSettings["LoopClose.nCovisibilityConsistencyTh"];
}
...
void LoopClosing::CorrectLoop() {
  // g_pause = true;
  //cout << "Loop detected --> go to correct " << endl;
  ...
  //cout << "Finsihed " << __FUNCTION__ << "set mLastLoopKFid to "
      //  << mLastLoopKFid << endl;
}
...
void LoopClosing::ResetIfRequested() {
  unique_lock<mutex> lock(mMutexReset);
  if (mbResetRequested) {
    std::cout << "=========================== ML--pLoopKeyFrameQueue will be clear " << std::endl;
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF) {
  cout << "[LC] Starting Global Bundle Adjustment" << endl;

  int idx = mnFullBAIdx;
  Optimizer::GlobalBundleAdjustemnt(mpMap, 10, false, &mbStopGBA, nLoopKF,
                                    false);
  ...
  // tree
  {
    ...
    if (!mbStopGBA) {
      ...
      // cout << __FUNCTION__ << " got " << lpKFtoCheck.size()
      //      << " frame to check " << endl;
      while (!lpKFtoCheck.empty()) {
        KeyFrame* pKF = lpKFtoCheck.front();
        const set<KeyFrame*> sChilds = pKF->GetChilds();
        cv::Mat Twc = pKF->GetPoseInverse();
        if (pKF->mbInitKF && pKF->mTcwGBA.empty()) {
          if(pKF->GetParent()!=NULL && !pKF->GetParent()->mTcwGBA.empty()){
            KeyFrame* cur_parent = pKF->GetParent();
            cv::Mat mTcwBefGBA = cur_parent->mTcwBefGBA; //parent's mTcw
            cv::Mat cur_BefGBA = pKF->GetPose();//cur mTcw
            cv::Mat Tcp = cur_BefGBA*mTcwBefGBA.inv();
            pKF->mTcwGBA = Tcp*pKF->mTcwGBA;
             cout << " [loopclose] Calculate KF " <<  pKF->mnId <<" 's TcwGBA by initKF's Parent " << cur_parent->mnId << endl;
          }else{
            pKF->mTcwGBA = pKF->GetPose();
            cout << " [loopclose] unitialized priginKF " << pKF->mnId << endl;
          }
        }
      ...
      cout << __FUNCTION__ << " get all map points " << vpMPs.size() << endl;
    ...
    mbRunningGBA = false;
    // g_pause = false;
    // cout << "g_pause set to false " <<endl;
  }
}
