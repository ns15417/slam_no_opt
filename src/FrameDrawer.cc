...
void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText) {
  ...
  if (nState == Tracking::NO_IMAGES_YET)
    ...
    s << " TRYING TO INITIALIZE " << "mCurFrameId: " << mCurFrameId;
    ...
    s << "KFs: " << nKFs << ", MPs: " << nMPs << ", mnMaxKFid: " << mnMaxKFid << ", mCurFrameId: " << mCurFrameId;
    if (mnTrackedVO > 0) s << ", + VO matches: " << mnTrackedVO;
  } else if (nState == Tracking::LOST) {
    s << " TRACK LOST. TRYING TO RELOCALIZE " << ", mCurFrameId: " << mCurFrameId;
...
void FrameDrawer::Update(Tracking *pTracker) {
  ...
  mCurFrameId = pTracker->mCurrentFrame.mnId;
  mnMaxKFid = mpMap->GetMaxKFid();
