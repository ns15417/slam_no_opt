...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB, int sensor_type, cv::Mat Tbc_in)
    : mnFrameId(F.mnId),
      ...
      mvP3M(F.mvP3M),
      mvP3MRight(F.mvP3MRight),
      mvMatcheslr(F.mvMatcheslr),
      mvP3Dfisheye(F.mvP3Dfisheye),
      mBowVec(F.mBowVec),
      mFeatVec(F.mFeatVec),
  ...
  mSensor_type = sensor_type;

  mTbc_KF = cv::Mat(4, 4, CV_32F);
  Tbc_in.copyTo(mTbc_KF);
...
KeyFrame::KeyFrame(InitKeyFrame &initkf, Map *pMap, KeyFrameDatabase *pKFDB,
    ...
    : mnFrameId(initkf.mnFrameId),
      ...
      mpMap(pMap),
      mpDRKeyFrame(NULL),
      mbDRKF(initkf.mbDRKF),
      mbInitKF(initkf.mbInitKF),
      mDrX(initkf.fDrX),
      mDrY(initkf.fDrY) {
  nNextId++;
  ...
  for (int i = 0; i < mnGridCols; i++) {
    mGrid[i].resize(mnGridRows);
    for (int j = 0; j < mnGridRows; j++) mGrid[i][j] = initkf.vGrid[i][j];
  }
  mbScaled = true;  // TODO: not sure whether should set to false or true
                    // especially for loaded frames
  mDistScale = 1.0;
  mTbc_KF = initkf.Tbc.clone();;
}

/*
专为LOadMap后新的KeyFrame设置初始值
*/
void KeyFrame::setKeyFrame(long unsigned int i) { nNextId = i + 1; }
...
void KeyFrame::SetPose(const cv::Mat &Tcw_) {
  ...
  cv::Mat cur_Tcb = mTbc_KF.inv();
  // robot center
  cv::Mat baselin_in_c(3, 1, CV_32F);  // X=-Y; Y=-Z;Z=X
  cv::Mat Rbc(3, 3, CV_32F);
  cv::Mat tbc(3, 1, CV_32F);
  mTbc_KF.rowRange(0, 3).colRange(0, 3).copyTo(Rbc);
  mTbc_KF.col(3).rowRange(0, 3).copyTo(tbc);
  baselin_in_c = -Rbc.t() * tbc;

  cv::Mat baselink_in_world = Rwc * baselin_in_c + Ow;
  //cv::Mat baselink_in_map = Rbc * baselink_in_world + tbc;
  //RobotCw = baselink_in_map.clone();
  // cout<<"[KF] RobotCw"<<RobotCw<<endl;
  RobotCw = baselink_in_world.clone();
}
...
cv::Mat KeyFrame::GetRobotCenter() {
  unique_lock<mutex> lock(mMutexPose);
  return RobotCw.clone();
}
...
void KeyFrame::UpdateConnections() {
...
#if 1
  // DR!!!
  if (mpDRKeyFrame != NULL) {
    // mpDRKeyFrame->AddConnection(this, 1000);
    this->AddConnection(mpDRKeyFrame, 1000);
    cout << "[KeyFrame] AddConnection" << endl;
    KFcounter[mpDRKeyFrame] = 1000;
  }
  // cv::waitKey();

  // This should not happen
  if (KFcounter.empty()) {
    cout << "[KeyFrame] KFcount is empty" << endl;
...

  {
    ...
    if (mpParent==NULL && mbInitKF==false) {
...
void KeyFrame::SetBadFlag() {
  ...
  for (map<KeyFrame *, int>::iterator mit = mConnectedKeyFrameWeights.begin(),
    ...
    if (mit->first == nullptr) {
      continue;  // shinan added for DR KeyFrame
    }
    ...
    if (mvpMapPoints[i]) mvpMapPoints[i]->EraseObservation(this);
  {
    ...
    if (mpParent != NULL)  // DR!!! to fix segment fault
      ...
      for (set<KeyFrame *>::iterator sit = mspChildrens.begin(),
        ...
        for (size_t i = 0, iend = vpConnected.size(); i < iend; i++) {
          for (set<KeyFrame *>::iterator spcit = sParentCandidates.begin(),
               ...
               spcit != spcend; spcit++) {
            /*if (vpConnected[i] == nullptr) {
              cout << "vpConnected[" << i << "] is empty " << endl;
              continue;
            }*/
...
cv::Mat KeyFrame::UnprojectStereo(int i) {
  ...
  cv::Mat Rwc = Tcw.colRange(0,3).rowRange(0,3).t();
  cv::Mat twc = -Rwc*Tcw.col(3).rowRange(0,3);
  if (z > 0) {
...
void KeyFrame::AddMapPoint(MapPoint *mp) {
  mvpMapPoints.push_back(mp);
  N++;
}
void KeyFrame::AddDRKeyFrame(KeyFrame *kf) {
  mpDRKeyFrame = kf;
  mbDRKF = true;
}

bool KeyFrame::isDRKF() {
  unique_lock<mutex> lock(mMutexConnections);
  return mbDRKF;
}
bool KeyFrame::isFalseKF(){
  return mbFalseKF;
}

void KeyFrame::SetScaleFlag() { mbScaled = true; }

bool KeyFrame::IsScaled() { return mbScaled; }

KeyFrame *KeyFrame::GetDRKeyFrame() { return mpDRKeyFrame; }

cv::Mat KeyFrame::GetOdomPoseinWorld() {
  unique_lock<mutex> lock(mMutexPose);
  float Drx = mDrX;
  float Dry = mDrY;
  cv::Mat OdomPose_baselink(3, 1, CV_32F);
  OdomPose_baselink.at<float>(0) = Drx;
  OdomPose_baselink.at<float>(1) = Dry;
  OdomPose_baselink.at<float>(2) = 0;
  cv::Mat Rbc(3, 3, CV_32F);
  cv::Mat tbc(3, 1, CV_32F);
  mTbc_KF.colRange(0, 3).rowRange(0, 3).copyTo(Rbc);
  mTbc_KF.col(3).rowRange(0, 3).copyTo(tbc);

  cv::Mat Rcb = Rbc.t();
  cv::Mat tcb = -Rcb * tbc;
  cv::Mat OdomPose = Rcb * OdomPose_baselink + tcb;
  cv::Mat Twb = cv::Mat::eye(4, 4, CV_32F);
  OdomPose.copyTo(Twb.col(3).rowRange(0, 3));
  return Twb;
}
