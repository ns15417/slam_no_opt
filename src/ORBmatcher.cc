...
bool ORBmatcher::CheckDistEpipolarLine3D(const cv::Point3f &P3M1,
                                         const cv::Point3f &P3M2,
                                         const cv::Mat &E12,
                                         const KeyFrame *pKF2,
                                         const cv::KeyPoint &kp2) {
  // Epipolar line in second image l = x1'F12 = [a b c]
  const float a = P3M1.x * E12.at<float>(0, 0) + P3M1.y * E12.at<float>(1, 0) +
                  P3M1.z * E12.at<float>(2, 0);
  const float b = P3M1.x * E12.at<float>(0, 1) + P3M1.y * E12.at<float>(1, 1) +
                  P3M1.z * E12.at<float>(2, 1);
  const float c = P3M1.x * E12.at<float>(0, 2) + P3M1.y * E12.at<float>(1, 2) +
                  P3M1.z * E12.at<float>(2, 2);
...
int ORBmatcher::SearchByProjection(KeyFrame *pKF, cv::Mat Scw,
  ...
  for (int iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++) {
    ...
    // Project into Image
    cv::Point2f uv;
    pKF->mpCamera->world2Img(p3Dc,uv);
...
int ORBmatcher::SearchForTriangulation(
    KeyFrame *pKF1, KeyFrame *pKF2, cv::Mat E12,
  ...
  cv::Point2f uv2;
  pKF1->mpCamera->world2Img(C2,uv2);
  const float ex = uv2.x;
  const float ey = uv2.y;
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints,
  ...
  for (int i = 0; i < nMPs; i++) {
    ...
    for (vector<size_t>::const_iterator vit = vIndices.begin(),
      ...
      if (pKF->mvuRight[idx] >= 0) { //在右目有数据
        // Check reprojection error in stereo
        if(pKF->mpCamera2){
          cv::Mat p3Dc2 = pKF->mRrl*p3Dc+pKF->mtlinr;
          cv::Point2f uv2;
          pKF->mpCamera2->world2Img(p3Dc2,uv2);
          const float u2 = uv2.x;
          const float v2 = uv2.y;
          const float ex = u2 - pKF->mvuRight[idx];
          const float ey = v2 - pKF->mvvRight[idx];
...
int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2,
  ...
  for (int i1 = 0; i1 < N1; i1++) {
    ...
    cv::Mat p3Dw = pMP->GetWorldPos();
    cv::Mat p3Dc1 = R1w * p3Dw + t1w;//point in pkf1
    cv::Mat p3Dc2 = sR21 * p3Dc1 + t21;//point in pkf2

    // Depth must be positive
    if (p3Dc2.at<float>(2) < 0.0) continue;

    cv::Point2f uv;
    pKF2->mpCamera->world2Img(p3Dc2, uv);
  ...
  for (int i2 = 0; i2 < N2; i2++) {
    ...
    cv::Mat p3Dw = pMP->GetWorldPos();
    cv::Mat p3Dc2 = R2w * p3Dw + t2w;//point in kf2
    cv::Mat p3Dc1 = sR12 * p3Dc2 + t12;//transfer to point in kf1

    // Depth must be positive
    if (p3Dc1.at<float>(2) < 0.0) continue;

    cv::Point2f uv;
    pKF1->mpCamera->world2Img(p3Dc1, uv);
  ...
  for (int i = 0; i < LastFrame.N; i++) {
    ...
    if (pMP) {
      if (!LastFrame.mvbOutlier[i]) {
        ...
        cv::Point2f uv;
        CurrentFrame.mpCamera->world2Img(x3Dc,uv);
