...
public:
    ...
    Frame(const cv::Mat &imGray, const double &timeStamp,
          ...
          cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera *pCamera, float dr_x,
          float dr_y, bool odom_flag, int sensor_type,
          cv::Mat Tbc);
    ...
    // Returns the robot center.
    inline cv::Mat GetRobotCenter(){
        return mRobotCw.clone();
    }
    ...
    void ComputeStereoFromRGBD(const cv::Mat &imDepth);
     ...
     //计算鱼眼镜头的双目匹配
    void ComputeFisheyeStereoMatches();

    //int Check3Dloc(std::vector<cv::Point3f> left_P3M,int iL, vector<int> vnMatcheslr,cv::Mat &p3dC1);
    int Check3Dloc(std::vector<cv::Point3f> left_P3M, int iL, std::vector<cv::Point3f> right_P3M,
                   int iR, cv::Mat &p3dC1);

    // Backprojects a keypoint (if stereo/depth info available) into 3D world coordinates.
    cv::Mat UnprojectStereo(const int &i);

    cv::Mat ProjectEUCM(float &camx, float &camy,cv::Mat &intrin_K);
...
public:
    ...
    std::vector<cv::Point3f> mvP3Dfisheye; //用于存放用左右相机和Rt三角化计算出的特征点的世界坐标

    std::vector<int> mvMatcheslr;
